...
export class AddApplicationAndDocument1676049351147 implements MigrationInterface {
    name = 'AddApplicationAndDocument1676049351147'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "applications" ("created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "application_id" uuid NOT NULL DEFAULT uuid_generate_v4(), "applicant_name" character varying NOT NULL, "applicant_surname" character varying NOT NULL, "applicant_email" character varying NOT NULL, "applicant_phone_number" character varying NOT NULL, "applicant_sex" "public"."applications_applicant_sex_enum" NOT NULL, "applicant_country" character varying NOT NULL, "applicant_birth_date" date NOT NULL, "applicant_residence_visa_avalibility" boolean NOT NULL, "applicant_preferred_direction_of_study" "public"."applications_applicant_preferred_direction_of_study_enum" NOT NULL, CONSTRAINT "UQ_ef282c002c2b46685ca2bbf6309" UNIQUE ("applicant_email"), CONSTRAINT "UQ_26ce735cc86620a0fcc88de9878" UNIQUE ("applicant_phone_number"), CONSTRAINT "PK_418038704e50c663590feb7f511" PRIMARY KEY ("application_id")); COMMENT ON COLUMN "applications"."created_at" IS 'Created at date'; COMMENT ON COLUMN "applications"."updated_at" IS 'Updated at date'; COMMENT ON COLUMN "applications"."application_id" IS 'Application''s unique identifier'; COMMENT ON COLUMN "applications"."applicant_name" IS 'Applicant''s name'; COMMENT ON COLUMN "applications"."applicant_surname" IS 'Applicant''s surname'; COMMENT ON COLUMN "applications"."applicant_email" IS 'Applicant''s email'; COMMENT ON COLUMN "applications"."applicant_phone_number" IS 'Applicant''s phone number with WatsApp'; COMMENT ON COLUMN "applications"."applicant_sex" IS 'Applicant''s sex'; COMMENT ON COLUMN "applications"."applicant_country" IS 'Applicant''s country'; COMMENT ON COLUMN "applications"."applicant_birth_date" IS 'Applicant''s birth date YYYY-MM-DD'; COMMENT ON COLUMN "applications"."applicant_residence_visa_avalibility" IS 'Applicant has residence visa'; COMMENT ON COLUMN "applications"."applicant_preferred_direction_of_study" IS 'Applicant''s preferred direction of study'`);
